---
title: "Lab 06: Sampling distribution and simulations"
...
<!-- Let's generate a random sample from a normal distribution with a mean of 0 and standard deviation of 1.

```{r}
set.seed(123)  # Ensuring reproducibility
sample_size = 100
random_sample = rnorm(sample_size, mean = 0, sd = 1)
```

[Visualizing the Distribution]{.hi}

We can visualize this sample to understand its distribution.

```{r}
library(ggplot2)

ggplot() + 
  geom_histogram(aes(x = random_sample), binwidth = 0.25, fill = 'blue', alpha = 0.4, color = 'white') +
  labs(title = "Histogram of Random Sample from Normal Distribution",
       x = "Value",
       y = "Frequency")
```
...
In real-world scenarios, we often deal with samples rather than the entire population. A sampling distribution is the distribution of a statistic (like mean, median, etc.) based on all possible samples that can be drawn from a population.

#### Simulating Sampling Distributions
We will simulate the process of drawing samples from a population and calculate the mean for each sample.

```{r}
population = rnorm(10000, mean = 0, sd = 1)  # Simulating a population
sample_means = replicate(1000, mean(sample(population, size = sample_size)))

ggplot() + 
  geom_histogram(aes(x = sample_means), bins = 30, fill = 'green', color = 'black') +
...
#### Conclusion
This lab introduced the basics of simulations in R. We explored how to generate random samples and visualize them, and we briefly touched on the concept of sampling distributions.
...
**Question:**  
You are given a population that is normally distributed with a mean of $\mu = 50$ and a standard deviation of $\sigma = 10$. Your task is to understand how the sampling distribution of the mean behaves compared to the population distribution.

1. Use R to generate a population of size 10,000 with the given parameters.
2. Draw 1,000 random samples from this population, each of size 30. Calculate the mean of each sample.
3. Plot the histogram of the sample means. Compare it with the population distribution in terms of shape, center, and spread.
4. Based on your observations, explain how the Central Limit Theorem applies to the sampling distribution of the mean.
5. Perform a simple hypothesis test: Test if the mean of the population is significantly different from 48. Use a significance level of 0.05.

Provide your R code and a brief explanation for each step. Your answer should illustrate the relationship between population distributions, sampling distributions, and hypothesis testing.

---

This draft and homework question should provide a good starting point for teaching these concepts in an introductory econometrics class. The R code is kept simple and uses basic functions, making it accessible to beginners. -->